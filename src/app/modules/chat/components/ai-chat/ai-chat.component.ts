...
export class AIChatComponent implements OnInit {
    ...
    this.chatbotService.chat({ message: userText }).subscribe({
      next: (res) => {
        ...
        this.messages.push({
          ...
          time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          isMe: false,
          hasPerformanceCard: replyText.toLowerCase().includes('recommend') || replyText.toLowerCase().includes('stats') || replyText.toLowerCase().includes('upper body')
        });
        this.scrollToBottom();
      },
      error: (err) => {
  ...
  deleteChat() {
    if (confirm('Are you sure you want to clear the AI Trainer chat history?')) {
